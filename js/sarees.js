...
function sareeProductCard(s, style = 'shop') {
  ...
  const actions = style === 'bestseller' ? '' : `<div class="suruchi-product-actions">
          <button class="suruchi-action-btn primary add-to-cart">+ Add to cart</button>
          <button class="suruchi-action-btn wishlist-btn">♡</button>
        </div>`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (shopGrid && shopGrid.dataset.render === 'sarees') {
  ...
  if (featured) featured.innerHTML = ALL_SAREES.slice(0, 5).map(s => sareeProductCard(s, 'home')).join('');
  if (trending) trending.innerHTML = ALL_SAREES.slice(5, 10).map(s => sareeProductCard(s, 'home')).join('');
  if (newArrival) newArrival.innerHTML = ALL_SAREES.slice(10, 15).map(s => sareeProductCard(s, 'home')).join('');
